...
class FacialExpressionDetector:
    ...
    def detect_expression_deepface(self, image_array: np.ndarray) -> Dict:
        """Detect facial expression using simple OpenCV-based analysis"""
        try:
            # Convert to grayscale for analysis
            gray = cv2.cvtColor(image_array, cv2.COLOR_BGR2GRAY)

            # Simple expression detection based on basic image features
            # This is a simplified version without DeepFace dependency
            ...
            # Determine dominant emotion based on image characteristics
            if std_brightness > 50:  # High contrast might indicate surprise or anger
                dominant_emotion = 'surprise' if mean_brightness > 150 else 'anger'
            elif mean_brightness < 100:  # Dark image might indicate sadness
                dominant_emotion = 'sad'
            elif mean_brightness > 180:  # Bright image might indicate happiness
                dominant_emotion = 'happy'
            else:
                dominant_emotion = 'neutral'

            # Adjust emotion scores based on dominant emotion
            for emotion in emotions:
                if emotion == dominant_emotion:
                    emotions[emotion] = 60.0
                else:
                    emotions[emotion] = max(5.0, emotions[emotion] - 10.0)
